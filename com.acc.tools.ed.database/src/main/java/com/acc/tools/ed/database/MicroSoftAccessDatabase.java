...
final public class MicroSoftAccessDatabase {
	...
	private final Logger log = LoggerFactory
			.getLogger(MicroSoftAccessDatabase.class);

	private static final String accessDBFile = "C:/Karthik/EDB/DB/EDB_New.accdb";

	private Connection edbConnection;

	/**
	 * This method will copy the MS Access linked tables with sharepoint from
	 * application to temporary directory and open JDBC connection.
	 * 
	 * @return
	 * @throws IOException
	 * @throws SQLException
	...
	public Connection getConnection() throws IOException, SQLException {
		...
		if (this.edbConnection == null) {
			...
			ProtectionDomain domain = MicroSoftAccessDatabase.class
					.getProtectionDomain();
			URL location = domain.getCodeSource().getLocation();
			String path = location.getPath();
			String dbpath = path.substring(1, path.lastIndexOf("/lib/"))
					+ "/classes/" + accessDBFile;
